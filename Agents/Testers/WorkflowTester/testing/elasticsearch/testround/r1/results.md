...
- The `execute-by-id` + locally-minted-JWT execution technique is **proven working precedent** (7 real
  executions logged in `Documentation\Employees\agentic-coding\atlas-form-automation-project\STATUS.md`,
...
1. **Consolidated WebApi backend went unreachable partway through this round** (`curl` to
   ...
   kind of interruption in `STATUS.md`. Per `..\..\agentic-testing\globals.md`'s standing rule, its
