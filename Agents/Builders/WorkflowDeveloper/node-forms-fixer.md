...
Reusable agent definition. Written 2026-09-20 from the SQL Server case (see `lessons\README.md`).
Read `resource.md` first for paths, connections and tool caveats; this file is the procedure.
...
| "Node is missing from the palette entirely" | Partly — run the audit (`rag\audit-all-datatemplates.md`) for duplicate/absent template rows |
...
1. Read `rag\form-resolution-pipeline.md` and `rag\profilename-to-form-contract.md` if you have not this session.
...
type-13 template count / a known template ID against each DB (`rag\database-topology.md`). Do this BEFORE
...
`rag\caching-layers.md` (10-minute template cache, output cache, 60-minute workflow cache -> restart WebApi,
...
### Step 7 — Log
Append a dated entry to this folder's `DevelopmentHistoryLog.md` (what/why/evidence) and a lesson to
`lessons\README.md` if something new was learned. Update `rag\node-type-status-table.md` for the node type.
...
7. **Cache awareness:** a fix that "does not show" is unproven, not failed — walk `rag\caching-layers.md` before re-fixing.
...
- [ ] Scripts saved in the DB project folder (uncommitted unless asked), `DevelopmentHistoryLog.md` and
